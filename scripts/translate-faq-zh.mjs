...
const PROGRESS_FILE = path.join(ROOT, '.states', 'translate-faq-zh-progress.json');
const EN_FAQ_PATH = path.join(ROOT, '.states', 'en-faq-source.json');
const SEO_PATH = path.join(ROOT, 'locales', 'zh-seo.json');
...
const TRANSLATION_PROMPT = `You are translating FAQ questions and answers for CreditStud.io from English to Simplified Chinese (zh-CN) for a US-based financial website.
...
- Use natural Simplified Chinese for US Chinese-speaking audience:
  - credit score → 信用分, balance transfer → 余额转移, annual fee → 年费
  - minimum payment → 最低还款, signup bonus → 开卡奖励, cash back → 返现
  - debt snowball → 雪球法, debt avalanche → 雪崩法
  - interest rate → 利率, APR → 年利率, credit limit → 信用额度
  - foreign transaction fee → 外币交易费, statement credit → 账单抵扣
...
async function translatePageFAQs(page, enFaqs) {
  ...
  const result = await callVenice([
    ...
    { role: 'user', content: `Translate these ${enFaqs.length} FAQs to Simplified Chinese (zh-CN):\n\n${faqLines}` }
  ]);
...
async function main() {
  ...
  for (const page of toDo) {
    ...
    try {
      ...
      // FAQ translations go into zh-seo.json under "faq" key
      for (let i = 0; i < translated.length; i++) {
