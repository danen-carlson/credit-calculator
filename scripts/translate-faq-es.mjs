#!/usr/bin/env node
// translate-faq-es.mjs — Re-translate Spanish FAQs using Gemini 3.1 Pro
// Usage: node scripts/translate-faq-es.mjs [--dry-run] [--max-pages N] [--only=page1,page2]
//
...
const PROGRESS_FILE = path.join(ROOT, '.states', 'translate-faq-progress.json');
...
async function translatePageFAQs(page, enFaqs) {
  ...
  const result = await callVenice([
    ...
    { role: 'user', content: `Translate these ${enFaqs.length} FAQs to US Spanish:\n\n${faqLines}` }
  ]);
...
async function main() {
  ...
  for (const page of toDo) {
    ...
    try {
      ...
      // FAQ translations go into es-seo.json under "faq" key
      for (let i = 0; i < translated.length; i++) {
        if (esSeo[page].faq && esSeo[page].faq[i]) {
          esSeo[page].faq[i].name = translated[i].name;
          esSeo[page].faq[i].acceptedAnswer = translated[i].acceptedAnswer;
        }
      }
  ...
  // Save es-seo.json
  await fs.writeFile(ES_SEO_PATH, JSON.stringify(esSeo, null, 2));
  console.log(`\n=== RESULTS ===`);
  console.log(`✅ Completed: ${completed}`);
  console.log(`❌ Failed: ${failed}`);
  console.log(`📝 Updated: ${ES_SEO_PATH}`);
}
